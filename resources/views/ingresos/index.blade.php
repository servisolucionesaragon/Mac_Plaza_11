...
<div class="d-flex align-items-center justify-content-between mb-4">
    <div>
        ...
        <p class="text-muted mb-0" style="font-size:13px;">
            Total del período: <strong style="color:#16a34a;">{{ $config->simbolo_moneda }} {{ number_format($totalPeriodo, 2) }}</strong>
        </p>
    </div>
    @if($cajaAbierta)
    ...
    <a href="{{ route('caja.index') }}" class="btn btn-warning px-4">
        <i class="fas fa-exclamation-triangle me-2"></i>Abrir Caja para Registrar Ingresos
    </a>
    @endif
</div>
...
<div class="card mb-4" style="border:1px solid #fde68a;background:#fffbeb;">
    <div class="card-body p-3" style="font-size:13.5px;color:#92400e;">
        <i class="fas fa-exclamation-triangle me-2"></i>No hay caja abierta. No se pueden registrar nuevos ingresos hasta abrir la caja del día.
    </div>
</div>
@endunless
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="ps-4" style="font-size:12px;color:#6b7280;">{{ $ingreso->fecha_ingreso->format('d/m/Y H:i') }}</td>
                        <td style="font-size:13.5px;">{{ $ingreso->descripcion }}</td>
                        <td style="font-size:13px;">{{ $ingreso->metodoPago->nombre ?? '—' }}</td>
                        <td style="font-size:13px;color:#6b7280;">{{ $ingreso->usuario->name ?? '—' }}</td>
                        <td style="font-size:13.5px;font-weight:600;color:#16a34a;">
                            + {{ $config->simbolo_moneda }} {{ number_format($ingreso->monto, 2) }}
                        </td>
                        <td class="text-end pe-4">
                            ...
                            <div class="d-flex gap-1 justify-content-end">
                                <button class="btn btn-sm btn-outline-secondary" style="border-radius:8px;padding:4px 10px;"
                                        ...
                                        onclick="abrirModalEditar({{ $ingreso->id }}, @json($ingreso->fecha_ingreso->format('Y-m-d\TH:i')), @json($ingreso->descripcion), {{ $ingreso->monto }}, {{ $ingreso->metodo_pago_id }}, @json($ingreso->notas))">
                                    <i class="fas fa-edit" style="font-size:12px;"></i>
                                </button>
                                <form action="{{ route('ingresos.destroy', $ingreso) }}" method="POST"
                                      onsubmit="return confirm('¿Eliminar este ingreso? Esta acción no se puede deshacer.')">
                                    @csrf @method('DELETE')
        ...
        <div class="modal-content" style="border-radius:16px;border:none;">
            <div class="modal-header" style="border-bottom:1px solid #f3f4f6;padding:20px 24px;">
                <h6 class="modal-title fw-bold">
                    <i class="fas fa-arrow-circle-up me-2" style="color:#16a34a;"></i>Nuevo Ingreso
                </h6>
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <input type="text" name="descripcion" class="form-control" value="{{ old('descripcion') }}"
                                   placeholder="Ej: Préstamo del propietario" required maxlength="255">
                        </div>
                        <div class="col-md-6">
...
<div class="modal fade" id="modalEditarIngreso" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius:16px;border:none;">
            <div class="modal-header" style="border-bottom:1px solid #f3f4f6;padding:20px 24px;">
                <h6 class="modal-title fw-bold">
                    <i class="fas fa-edit me-2" style="color:#16a34a;"></i>Editar Ingreso
                </h6>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form id="formEditarIngreso" action="" method="POST">
                @csrf @method('PUT')
                <div class="modal-body p-4">
                    <div class="row g-3">
                        <div class="col-12">
                            <label class="form-label">Fecha</label>
                            <input type="datetime-local" name="fecha_ingreso" id="editFecha" class="form-control">
                        </div>
                        <div class="col-12">
                            <label class="form-label">Descripción <span class="text-danger">*</span></label>
...
@push('scripts')
<script>
const baseIngresosUrl = '{{ url('/ingresos') }}';
...
function abrirModalEditar(id, fecha, descripcion, monto, metodoPagoId, notas) {
    ...
    document.getElementById('formEditarIngreso').action = baseIngresosUrl + '/' + id;
    new bootstrap.Modal(document.getElementById('modalEditarIngreso')).show();
}

@if($errors->any())
    document.addEventListener('DOMContentLoaded', function() {
        new bootstrap.Modal(document.getElementById('modalNuevoIngreso')).show();
    });
@endif
</script>
@endpush
