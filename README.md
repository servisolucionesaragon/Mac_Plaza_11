...
| **Catálogos** | Gestión (crear/editar/activar-desactivar/eliminar) de categorías, marcas, condición, almacenamiento, RAM y métodos de pago — antes eran listas fijas hardcodeadas, ahora son catálogos dinámicos y protegidos contra borrado si tienen productos/ventas asociadas. Además, se pueden crear **tipos de catálogo completamente nuevos** (ej. "Proveedores") con su propio ícono — elegido con un selector visual de íconos reales, no un texto — y sus valores quedan disponibles de inmediato en Inventario (ver más abajo). |
...
| `categorias`, `marcas`, `condiciones`, `almacenamientos`, `rams`, `metodos_pago` | Catálogos dinámicos (fijos, un `<select>` con FK dedicada en `productos`) |
...
| `productos` | Inventario. `stock` y `precio_compra` son columnas reales mantenidas por la app (no calculadas por subconsulta) a partir de los lotes — se recalculan en cada alta/consumo/devolución de lote para que Dashboard/Reportes/Export sigan funcionando sin cambios. |
| `lotes_producto` | Cada ingreso de mercancía de un producto: `cantidad_inicial`, `cantidad_restante`, `costo_unitario`, `proveedor` (texto, alimentado por el catálogo dinámico "Proveedores"), `fecha_ingreso`, `notas`, `user_id`. Base del costeo FIFO. |
| `detalle_venta_lote` | De qué lote(s) salió cada línea de una venta (`detalle_venta_id`, `lote_id`, `cantidad`, `costo_unitario` — snapshot al momento de la venta). Una sola línea puede repartirse entre varios lotes si la venta cruza el límite de existencias de uno. |
